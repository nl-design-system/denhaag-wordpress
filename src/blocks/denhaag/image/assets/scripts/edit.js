...
export default function Edit( { attributes, setAttributes, clientId } ) {
	const media = useSelect(
		( select ) => select( 'core' ).getMedia( attributes.image ),
		[ attributes.image ]
	);
	...
	const imageMetaData = useMemo( () => {
		if ( ! attributes.image || isLoading || ! media ) {
			return null;
		}
	...
	useLayoutEffect( () => {
		...
		if (
			!! attributes.caption &&
			!! attributes.caption.includes( '<meta charset="utf-8">' )
		) {
			setAttributes( {
								...
								!! imageMetaData && (
									...
									>
										...
										<span className={ _CLASSES.label }>
											{ _x(
												'Download image',
												'ncb-denhaag/image: Download label',
												'nlds-community-blocks'
											) }
										</span>
									</a>
